                    ...
                    <div {*class="col-md-6"*}>
                        <div class="form-group button-ts">
                            <input type="radio"
                                   ...
                                   name="time"
                                   value="{$item.time}"
                                   {if  ($item.booked and $item.time|strtotime != $time_booked|strtotime and $date_booked != $date)
                                        or $item.exceptionned
                                   }
                                       disabled
                                    {/if}
                                   {if ($item.booked and $item.time|strtotime == $time_booked|strtotime)
        ...
        <div class="col-md-6 afternoon-timeslots">
            ...
            <div class="timeslots afternoon">
                {foreach  key=key item=item from=$timeslots.afternoon}
                    <div {*class="col-md-6"*}>
{*                        {$date_booked|var_dump}
                        {$date|var_dump}
                        {$item.booked|var_dump}*}
                        <div class="form-group button-ts">
                            <input type="radio"
                                   id="ts-morning-{$key}"
                                   name="time"
                                   value="{$item.time}"
                                   {if ($item.booked and ($item.time|strtotime != $time_booked|strtotime or $date_booked != $date)) or $item.exceptionned}disabled{/if}
                                   {if ($item.booked and $item.time|strtotime == $time_booked|strtotime) and $date_booked == $date}checked="checked"{/if}
                            >
                            <label class="btn btn-default" for="ts-morning-{$key}">{$item.timeslot_string}</label>
                        </div>
                    </div>
